...
st.set_page_config(page_title='Bald Classification ', layout='centered')
st.title('Null Class Intership Project')
st.title("Bald Classification  Project")
...
if uploaded_file:
    ...
    if os.path.exists(output_image_path):
        st.image(output_image_path, caption="Bald Classification Result", use_column_width=True)
        with open("runs/classify/predict/labels/test_image.txt",'r') as f:
            a = f.readline()
            if('NotBald' in a):
                st.markdown(f"<h3 style='text-align: center;'>It's not a Bald.</h3>", unsafe_allow_html=True)
            else:
                st.markdown(f"<h3 style='text-align: center;'>It's a Bald.</h3>", unsafe_allow_html=True)
